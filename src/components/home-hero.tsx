    ...
    <section className="relative flex flex-col overflow-hidden border-b border-brand-green/15 md:min-h-[min(82vh,56rem)]">
      ...
      <div className="relative z-10 flex flex-col bg-brand-cream px-5 pb-11 pt-9 sm:px-6 sm:pb-12 sm:pt-10 md:absolute md:inset-0 md:min-h-[min(82vh,56rem)] md:justify-center md:bg-transparent md:pb-14 md:pt-16 lg:pt-20">
        ...
        <div className="mx-auto w-full max-w-3xl text-center lg:max-w-[52rem]">
          ...
          <h1
            className={cn(
              "font-accent text-[1.55rem] font-semibold leading-[1.2] tracking-tight text-brand-green-dark sm:text-[1.85rem] md:text-5xl md:leading-[1.1] md:text-white md:[text-shadow:0_1px_4px_rgba(0,0,0,0.95),0_5px_32px_rgba(0,0,0,0.42)] lg:text-[3.35rem] lg:leading-[1.08]",
              h.heroEyebrow.trim() ? "mt-4 sm:mt-5 md:mt-5" : "mt-0",
            )}
          >
            <span className="block">{h.heroTitle}</span>
            {h.heroTitleAccent.trim() ? (
          ...
          <div
            className={
              !h.heroLead.trim() && h.heroPoints.length === 0
                ? "mt-10 flex flex-col items-stretch justify-center gap-3 sm:mt-12 sm:flex-row sm:items-center sm:gap-4 md:mt-14"
                : "mt-8 flex flex-col items-stretch justify-center gap-3 sm:mt-10 md:mt-11 sm:flex-row sm:items-center sm:gap-4"
            }
          >
            <Link
              href="/products"
